...
public class ManageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String totalAmount = numberFormat.format(total);
        HttpSession session = request.getSession();
        try {
            tour = dao.getTheMostTour();
            session.setAttribute("theMostTour", tour);
        } catch (Exception ex) {
        }
        session.setAttribute("numberOfUsers", number);
        session.setAttribute("numberOfBooking", numberBooking);
        session.setAttribute("totalAmount", totalAmount);
        response.sendRedirect("html/manage.jsp");
    }
